...
export class LoginServiceService {
  ...
  loginAuth(userObj: any) {
   ...
   return this._http.post("http://portail.chatelet.dutmen.fr:3000/login", { uname: userObj.uname, upass: userObj.upass, uip: userObj.uip, ubrowser: userObj.ubrowser }, { observe: 'response', headers: this.headerOptions });
  }
  ...
  getAuth() {
    return this._http.get("http://portail.chatelet.dutmen.fr:3000/tfa/setup", { observe: 'response' });
  }
  getUser(user:String){
    return this._http.get(`http://portail.chatelet.dutmen.fr:3000/user/${user}`)
  }
  deleteAuth() {
    return this._http.delete("http://portail.chatelet.dutmen.fr:3000/tfa/setup", { observe: 'response' });
  }

  verifyAuth(token: any) {
    return this._http.post("http://portail.chatelet.dutmen.fr:3000/tfa/verify", { token }, { observe: 'response' });
  }
